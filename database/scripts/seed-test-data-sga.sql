...
-- Fecha: 2026-04-07 (actualizado 2026-04-29)
-- 
...
-- Columna correcta: total_weight_grams (no weight_per_unit_grams)
-- Proveedor correcto: JRS (no JRF, que no existe en la BD)
-- ==========================================
...
-- Muestra 4: Toxico (JRS - Farmacéutica)
INSERT INTO global_samples (
    name, supplier_id, lot, expiration_date, manufacture_date,
    ghs_danger_class, market_line_id, dimensions,
    total_units, available_units, total_weight_grams
...
SELECT 
    'Metanol HPLC Grade', s.id, 'JRS-2024-002',
    '2026-09-30', '2024-01-10',
    'Toxico', ml.id, '1x1x1',
    15, 15, 1500.00
FROM suppliers s, market_lines ml
WHERE s.name = 'JRS' AND ml.name = 'Farmacéutica'
ON CONFLICT DO NOTHING;
...
SELECT 
    ...
    40, 40, 6000.00
FROM suppliers s, market_lines ml
WHERE s.name = 'JRS' AND ml.name = 'Cosmética'
ON CONFLICT DO NOTHING;
