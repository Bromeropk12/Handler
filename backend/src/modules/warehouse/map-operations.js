...
function generateGridMatrix3D(shelf, samples) {
  ...
  samples.forEach(sample => {
    if (sample.position_x === null || sample.position_y === null || sample.position_z === null) {
      return;
    }
    const startX = sample.position_x;
    const startY = sample.position_y;
    const startZ = sample.position_z;
...
const moveSample = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { sample_id, new_position_x, new_position_y, new_position_z, target_shelf_id } = req.body;

    const current_shelf_id = id;
    const final_shelf_id = target_shelf_id || id;
    ...
    let shelf = await query('SELECT * FROM shelves WHERE id = $1', [current_shelf_id]);

    if (final_shelf_id !== current_shelf_id) {
        const targetShelf = await query('SELECT * FROM shelves WHERE id = $1', [final_shelf_id]);
        if (targetShelf.rows.length === 0) {
            throw new AppError('Anaquel destino no encontrado', 404);
        }
        if (shelf.rows[0].market_line_id !== targetShelf.rows[0].market_line_id) {
            throw new AppError('Solo se permiten movimientos dentro de la misma línea de mercado', 400);
        }
        shelf = targetShelf;
    }
